...
class Block(nn.Module):
    ...
    def forward(self, x, adapt=None):
        ...
        if adapt is not None:
            if isinstance(adapt, (list, nn.ModuleList)):
                adapts = list(adapt)
            else:
                adapts = [adapt]

        if adapts:
            adapt_x = adapts[0](x, add_residual=False)
            for a in adapts[1:]:
                adapt_x = adapt_x + a(x, add_residual=False)
        else:
            adapt_x = None
...
class VisionTransformer(nn.Module):
    ...
    def get_new_adapter(self):
        ...
        if config.ffn_adapt:
            if len(self.adapter_list) > 0:
                self.cur_adapter = copy.deepcopy(self.adapter_list[-1])
            else:
                self.cur_adapter = nn.ModuleList()
    ...
    def add_adapter_to_list(self):
        ...
        self.adapter_list.append(self.cur_adapter)
        self.get_new_adapter()
    ...
    def forward_cumulative(self, x, adapter_index):
        B = x.shape[0]
        x = self.patch_embed(x)
        ...
        if adapter_index < len(self.adapter_list):
            selected_adapter = self.adapter_list[adapter_index]
        else:
            selected_adapter = self.cur_adapter

        for j in range(len(self.blocks)):
            adapt = selected_adapter[j]
...
def vit_base_patch16_224_multi_adapter(pretrained=False, **kwargs):
